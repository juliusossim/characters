...
const initialContextValue = {
	mode: '',
	currentCharacterKeys: 1,
	currentCharacterPage: 1,
	setCurrentCharacterkeys: (a: number) => a,
	setCurrentCharacterPage: (a: number) => a,
	characterRef: null,
	setCurrentCharacters: (a: 1) => a,
	currentCharacters: [],
};
...
export const AppProviderContext =
	createContext<ProviderContextValueProps>(initialContextValue);

export const ProviderContextProvider: FC<ProviderContextProps> = ({
	children,
}) => {
	...
	const [currentCharacters, setCurrentCharacters] =
		useState<CurrentCharactersProps>([]);
	...
	return (
		<AppProviderContext.Provider value={values}>
			<Main variant={mode}>{children}</Main>
