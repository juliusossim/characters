import { StyledListItem, StyledIconContainer, StyledTextContainer } from "@/components/Styles/Sidebar";
...
export const NavItem = ({open, item, toggle }: NavItemProps) => {
  ...
  return (
    <WrapInLink open={open}  to={`${item.id}`} toggle={toggle}>
      ...
      whileTap={{ scale: 0.95 }}
    >
      <StyledIconContainer style={style} srcSet={item.img_url} alt={item.name} />
